...
def convert_file(input, output=None, archived=False, archivedoutput=None, from_folder=False, force_file=False):
    print(f"\n\nConverting file {input}")
    file_name, extension = os.path.splitext(os.path.basename(input))

    if extension != ".json" and not force_file:
        print("Skipping file, not json format. Use flag --force to force the file to be used. WARNING: This script may throw an error.")
        return
...
parser = argparse.ArgumentParser(
     ...
     description=textwrap.dedent('''\
         ...
         '''))
parser.add_argument('-i', '--input', dest='input', type=str, required=True, help='Path to input file or directory.')
parser.add_argument('-o', '--output', dest='output', type=str, help='Path to output file or directory.')
parser.add_argument('-r', '--recursive', dest='recursive', action='store_true', help='Directory only. Enable recursive convertion for directories. Not used for individual files. The subdirectories structures will be lost in the output folder.')
parser.add_argument('-a', '--archived', dest='archived', action='store_true', help='Directory only. Separate archived notes to a separate directory. Default directory "archived"')
parser.add_argument('-f', '--force', dest='force_file', action='store_true', help='Force the file to be read if the extension is not .json. This may break the conversion.')
parser.add_argument('--archivedoutput', dest='archivedoutput', type=str, help='Path to archived output directory.')
...
if __name__ == '__main__':
    ...
    if os.path.isdir(args.input):
        ...
        if args.archived:
            archive_path = os.path.join(args.output, args.archivedoutput) if args.archivedoutput else os.path.join(args.output, "archived")
            try:
        ...
        convert_folder(args.input, args.recursive, args.output, args.archived, args.archivedoutput, args.force_file)
